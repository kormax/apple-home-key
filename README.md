...
- NFC:
  - Authentication: runs on the SE and is based on the [Car Key](https://developer.apple.com/videos/play/wwdc2020/10006/) protocol, with minor changes, such as lack of direct pairing (which is done via HAP instead), changes to the KDF influcenced by multi-reader environment and other;
...
In order to guarantee privacy due to lack of reader authentication in FAST command, endpoint does not return identifyable data as-is.
...
During the STANDARD command, reader generates a signature over commonly known data in order to prove to the endpoint that it's safe to talk to, thus protecting against malicious readers. 
...
Starting from response of STANDARD command and onward, excluding OP CONTROL FLOW, a secure channel is used for communication.
...
Describing the implementation in full is out of scope of this document.
...
After a full attestation package has been retreived using ENVELOPE + GET RESPONSE, it has to be verified.

Verification process mimicks the ISO18013 claim verification process.
...
It also tells us that there's a third, publicly unknown "Access" implementation.  
